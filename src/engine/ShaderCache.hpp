...
public:
    ...
    // Stage 5: fused-chain cache. Same on-disk format (.spv keyed by
    // hash) plus a .key.json sidecar for equality re-check (the hash
    // is over a vector of strings, so collision is more likely than
    // for a per-node key).
...
private:
    ...
    // Sidecar: write full key fields next to .spv; on load, re-parse
    // and compare to the requested key. Mismatch = treat as miss.
