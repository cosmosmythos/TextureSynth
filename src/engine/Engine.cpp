...
uint64_t Engine::reroute_output(NodeId node_id) {
    TE_GUARD_READY(0);
    clear_error();

    if (current_graph_.nodes.empty()) {
        set_error_(EngineErrorCode::GraphValidation,
                   "reroute_output called before any set_graph", EnginePhase::GraphSubmit);
        return 0;
    }
    if (node_id == current_graph_.output_node) {
        return installed_generation_ ? installed_generation_ : pending_generation_;
    ...
    if (!node_exists) return 0;

    // Update the output target.
    current_graph_.output_node = node_id;

    // Re-validate to get new IR (active path changes with output node).
    auto ir_result = validate_graph(current_graph_, node_lib_);
    if (!ir_result.success) {
        log_warn("reroute_output validation: " + ir_result.error);
        current_graph_.output_node = 0;
        return 0;
    }
    ir_result.ir.graph_default_depth = graph_default_depth_;
    resolve_node_depths(ir_result.ir);

    auto compile_result = FusedGraphCompiler::compile(ir_result.ir, node_lib_, node_id);
    if (!compile_result.success) {
        set_error_(EngineErrorCode::GraphCompile, compile_result.error,
                   EnginePhase::GraphSubmit);
        current_graph_.output_node = 0;
    ...
    // Retire old passes and chains.
    retire_all_passes_();

    // Re-allocate resources (alias classes may have changed with new active path).
    std::string rerr;
    if (!resources_.allocate_for_graph(ctx_, current_ir_, node_lib_, output_w_, output_h_,
                                        texture_format_, &rerr,
                                        &compile_result.pass_plan.color_classes,
    ...
    // NOTE: We do NOT call seed_param_ssbo_defaults_() here.
    // Params are preserved from the previous graph — same nodes, same offsets.
    // This avoids the zero-then-repush flicker that set_graph() causes.
    ...
    for (size_t i = 0; i < compile_result.pass_plan.passes.size(); ++i) {
        const auto& pass = compile_result.pass_plan.passes[i];
        if (pass.kind != PassKind::Compute) continue;
        if (i < cip.size() && cip[i] != UINT32_MAX) continue;
        auto blob = cache_->load(pass.variant_key);
        if (!blob) { all_cached = false; break; }
        cached_spv[i] = std::move(*blob);
    }
    ...
    if (all_cached) {
        ...
        log_info("reroute_output: fast path, gen=" + std::to_string(gen)
                 + " output_node=" + std::to_string(node_id));
        return gen;
    }
    ...
    pending_final_output_ = compile_result.pass_plan.final_output_resource;
    pending_pass_plan_    = compile_result.pass_plan;
    log_info("reroute_output: async path, gen=" + std::to_string(gen)
             + " output_node=" + std::to_string(node_id));
    return gen;
}
