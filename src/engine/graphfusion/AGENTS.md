...
- **Multi-pass singleton chains**: nodes with `pass_count > 1` (e.g. blur) force a split BEFORE and AFTER them in the fusion compiler. They become singleton chains (`group.nodes.size() == 1`) with `sub_pass_count = type->pass_count`. The fused compiler emits per-sub-pass GLSL with different `ShaderVariantKey::specialization[0]` values. The chain's `chain.glsl` is set to the node-level emit (`pass.shader_glsl`) for backward compat (bypass path). Multi-pass nodes CANNOT fuse with other nodes — they need intermediate images between sub-passes. Without the trailing split, a multi-pass node followed by another node (e.g. blur→invert) would be grouped together, and `is_multipass` would be false (requires `group.nodes.size() == 1`), causing the node to run single-pass and produce stretched output.
