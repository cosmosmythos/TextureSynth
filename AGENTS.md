...
| Chain slot staleness (async upload) | `ChainExec::chain_in_sampled_slots[]` is snapshotted at `populate_chains_()` time. If an async image upload hasn't completed yet, the snapshot holds the dummy (black) slot. `poll_completed_uploads_()` patches per-pass `pe.in_sampled_slots[]` but NOT the chain snapshot. Fix: `ChainExec::slot_sources[]` maps each ext slot to `(member_pass_index, input_index)`; `record_chain_dispatch_()` reads `passes_[member_pass_idx].in_sampled_slots[input_index]` live. If you modify chain dispatch, always read live slots — never trust the snapshot. |
